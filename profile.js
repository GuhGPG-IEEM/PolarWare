...
function setupAddressForm(userId) { /* ...código existente, com pequenas alterações... */
    ...
    newAddressForm.addEventListener('submit', async (event) => {
        ...
        if (error) { console.error('Erro ao adicionar endereço:', error); alert('Não foi possível salvar o novo endereço.');
        } else { alert('Endereço adicionado com sucesso!'); newAddressForm.reset(); loadAddresses(userId); }
    });
    const addressList = document.getElementById('address-list');
    addressList.addEventListener('click', async (event) => {
        if (event.target.classList.contains('delete-address-btn')) {
            ...
            if (confirm('Tem certeza que deseja excluir este endereço?')) {
                ...
                if (error) { console.error('Erro ao excluir endereço:', error); alert('Não foi possível excluir o endereço.');
                } else { alert('Endereço excluído com sucesso.'); loadAddresses(userId); }
            }
        }
    });
}